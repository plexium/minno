...
{
    $id = empty($_GET['id']) ? 'index': (get_magic_quotes_gpc() ? $_GET['id'] : addslashes($_GET['id']) );
...
function i( $s = null, $echo = true )
{
   ...
   if ( !isset($stack[$s]) )
   {
      ...
      do
          if ( $_SESSION['auth'] && ( isset($_GET['edit']) || !$row ) && $s == ($_GET['id'] || $s == 'index') && $echo == true && $stack[$s] == 0 )
              echo '<form action="'.dirname($_SERVER['SCRIPT_NAME']).'/'.h($_GET['id']).'?" method="post"><textarea cols="80" rows="20" id="page" name="page">'.htmlspecialchars($row['page']).'</textarea><input type="submit" value="'.($row?'update':'create').'"></form>';
          else
              eval( " ?>{$row['page']}<?php " );
